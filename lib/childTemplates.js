...
function extendRequest (skipData, target, request) {
  ...
  let data
  if (skipData) {
    data = extend(false, {}, request.data)
    request.data = null
  }

  const req = extend(true, target, request)
