...
async function evaluateChildTemplates (reporter, request, response, evaluateInTemplateContent) {
  ...
  async function convert (str, p1, offset, s) {
    ...
    let template = await reporter.documentStore.collection('templates').findOne({
      name: templateName,
      ...(folder && { folder: { shortid: folder.shortid } })
    }, request)

    if (!template && folder) {
      template = await reporter.documentStore.collection('templates').findOne({
        name: templateName
      }, request)
    }
...
module.exports = function (reporter, definition) {
  ...
  reporter.beforeRenderListeners.add(definition.name, this, (request, response) => {
    return evaluateChildTemplates(reporter, request, response, true)
  })
  ...
  reporter.afterTemplatingEnginesExecutedListeners.add(definition.name, this, (request, response) => {
    return evaluateChildTemplates(reporter, request, response, false)
  })
